...
export function PlayerSheet({ player, onEdit, onDelete, onClose }: PlayerSheetProps) {
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <CardDescription className="font-serif text-base mt-2">
                {player.race} {player.class} - <span className="text-primary font-bold">Nível {player.level}</span>
              </CardDescription>
            </div>
          ...
          <Card className="bg-destructive/10 border-destructive/30">
            <CardContent className="pt-6">
              <div className="text-center">
                ...
                <p className="text-4xl font-bold text-destructive">{player.hp}</p>
              </div>
            </CardContent>
          </Card>
                ...
                <CardContent className="p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {player.inventory.map((item, index) => (
                      <div key={index} className="flex items-center gap-2 p-2 rounded bg-card">
          ...
          {player.notes && (
            <div>
              ...
              <Card className="bg-muted/30">
                <CardContent className="p-4">
                  <p className="font-serif leading-relaxed text-pretty">{player.notes}</p>
                </CardContent>
              </Card>
            </div>
          )}
