...
export default function ClassFeaturesSection({
  ...
  const subclassFeatures = selectedSubclass
    ? cumulativeFeatures.filter((f: any) => {
        return (
          f.index.includes(selectedClass.index) &&
          f.index.includes(subclassIndex)
        );
      })
    : [];
            ...
            {classOnlyFeatures.length > 0 && (
              <div className="space-y-4">
                {classOnlyFeatures.map((feature: any) => (
                  ...
                  >
                    <h4 className="font-semibold text-martial-400 mb-2">
                      {feature.name}
                    </h4>
                ...
                <div className="space-y-4">
                  {subclassFeatures.map((feature: any) => (
                    <div
                      ...
                      className="p-4 bg-bg-inset rounded border border-border-default"
                    >
                      <h4 className="font-semibold text-nature-400 mb-2">
                        {feature.name}
                      </h4>
                      <div className="text-sm text-text-secondary">
                        {feature.desc && Array.isArray(feature.desc) ? (
