...
export function PlayerView({ campaignId }: { campaignId: string }) {
  ...
  const filteredPlayers = gameData.playableCharacters.filter(
    (player: PlayableCharacter) => {
      ...
      const matchesClass =
        filterClass === "all" || player.className === filterClass;
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {filteredPlayers.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPlayers.map((player: PlayableCharacter) => (
              <Card
                key={player.id}
                className="parchment-texture metal-border hover:glow-gold transition-all cursor-pointer"
                onClick={() =>
                  router.push(`/campaign/${campaignId}/character/${player.id}`)
                }
              >
                <CardHeader>
