...
function formatMonsterNotes(apiMonster: GetMonsterQuery["monster"]): string {
  ...
  const acValues = apiMonster.armor_class
    ?.map((ac) => {
  ...
  const lines = [
    ...
    "",
    "Resistências:",
    ...(apiMonster.damage_resistances?.length
      ? apiMonster.damage_resistances
      : ["Nenhuma"]),
    "",
    "Imunidades:",
    ...(apiMonster.damage_immunities?.length
      ? apiMonster.damage_immunities
      : ["Nenhuma"]),
    "",
    "Ações:",
